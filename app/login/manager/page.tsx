...
import { Users, Mail, ArrowRight, Clock } from 'lucide-react';
...
export default function ManagerLoginPage() {
  ...
  const handleSendOTP = async (e: React.FormEvent) => {
    ...
    try {
      console.log('Sending Manager OTP to:', email.trim().toLowerCase());
      ...
      console.log('Manager OTP response:', { data, error });

      if (error) throw error;

      setStep('otp');
    } catch (err) {
      console.error('Send Manager OTP error:', err);
  ...
  const handleVerifyOTP = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('Verify Manager OTP response:', { data, error });
      ...
      if (data.user) {
        ...
        console.log('Manager verified, redirecting...');
        router.push('/dashboard/manager');
      }
    } catch (err) {
      console.error('Verify Manager OTP error:', err);
      ...
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <p className="text-foreground-muted mt-2">
            Sign in to manage schedules and workers
          </p>
        </div>
