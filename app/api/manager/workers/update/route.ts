...
export async function PUT(request: Request) {
  try {
    const { workerId, positionIds, placeIds, hourly_rate, positionRatings, status, start_date, worker_rating, can_open, can_close } =
      await request.json();
    ...
    if (status) {
      ...
      updateFields.is_active = status === 'ACTIVE';
    }
    ...
    if (worker_rating !== undefined) {
      updateFields.worker_rating = worker_rating;
    }
    ...
    if (placeIds !== undefined) {
      ...
      await supabase.from("worker_places").delete().eq("worker_id", workerId);
